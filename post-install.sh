...
if [ ! -d $FSF_LOGDIR ]; then mkdir -m 644 $FSF_LOGDIR && chown -R fsf:fsf $FSF_LOGDIR
fi

# set up $FSF_ARCHIVEDIR

if [ ! -d $FSF_ARCHIVEDIR ]; then mkdir -m 644 $FSF_ARCHIVEDIR && chown -R fsf:fsf $FSF_ARCHIVEDIR
fi
